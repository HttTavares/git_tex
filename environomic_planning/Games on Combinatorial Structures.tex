...
\begin{definition}
	...
	\begin{center}
		...
		%\{\phi(Y) \ | \ \text{for some} Y \in \cay \ \text{such that} w(Y) is \}$
	\end{center}
...
%Notice that we define weights as being present in both nodes and edges, whereas most time weights are present in only one of them. 
...
\begin{proposition}
	...
	\begin{enumerate}
		%\item There is a single $s \in X^0$ such that $prev(s) = s$.
